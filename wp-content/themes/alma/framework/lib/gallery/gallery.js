...
jQuery(function(){
	...
	var win = jQuery(window),
		loading = jQuery('.template-gallery-loading'),
		gallery = jQuery('.template-gallery'),
		lightwindow = jQuery('.template-gallery').attr('data-window'), //Set the lightwindow
		data_images = jQuery('.template-gallery').attr('data-images'); //Set the images json var


	var gallery_images = eval(data_images);

	// response.data holds the photos
	response = jQuery.parseJSON(gallery_images);
	...
	gallery.on('data-ready window-resized page-turned', function(event, direction){
		...
		jQuery.each(set, function(){
			...
			// build the cache
			if ( lightwindow != "") {
				cache.push('<a href="' + this.large + '" class="'+ (lightwindow=="swipebox" ? lightwindow : '') +'"' +
					'style="width:' + picture_width + 'px;height:' + picture_height + 'px;background-image:url(' + this.thumb + ')">'+
					 '<img src="'+ this.thumb +'" width="' + picture_width + '" height="' + picture_height + '">' +
					'</a>');
			} else {
				cache.push('<a ' +
					'style="width:' + picture_width + 'px;height:' + picture_height + 'px;background-image:url(' + this.thumb + ')">'+
		...
		jQuery.when.apply(jQuery, deferreds).always(function(){
			...
			if(event.type == 'window-resized'){
				...
				//jQuery('.template-gallery .swipebox').swipebox();

				if (lightwindow=="swipebox")
				{
					jQuery('.template-gallery .swipebox').swipebox();
				}

				if (lightwindow=="magnificPopup")
				{

					jQuery('.template-gallery.magnificPopup').magnificPopup({
						delegate: 'a[href]',
						type: 'image',
						...
						zoom: {
							...
							opener: function(element) {
								return element.find('img').parent();
							}
						}
					});
				}
	...
	win.on('resize', function(e){
		...
		var width = jQuery('.template-gallery-wrapper').width(),
			height = jQuery('.template-gallery').height(),
			gallery_width,
			gallery_height,
			difference;
