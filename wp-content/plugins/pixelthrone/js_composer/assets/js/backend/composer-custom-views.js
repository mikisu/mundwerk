...
/* PixelThrone Start*/
        ...
        changeShortcodeParams:function (model) {
          ...
          this.$el.find('> .controls .pt_row_viewport').remove();

          if(!_.isEmpty(params.pt_hidden_viewport)) {

                PT_htmlOut ='';
                if (params.pt_hidden_viewport.indexOf("hidden-desktop") != -1)   PT_htmlOut +='<i class="ion-monitor"></i>';
                if (params.pt_hidden_viewport.indexOf("hidden-tablet")  != -1)   PT_htmlOut +='<i class="ion-ipad"></i>';
                if (params.pt_hidden_viewport.indexOf("hidden-phone")   != -1)   PT_htmlOut +='<i class="ion-iphone"></i>';
                
                $('<span class="pt_row_viewport">' + PT_htmlOut +'</span>').insertAfter($column_edit);

          }

          if(!_.isEmpty(params.pt_vc_bg_color)) {
              $('<span class="vc_row_color" style="background-color: ' + params.pt_vc_bg_color + '" title="' + i18nLocale.row_background_color + '"></span>').insertAfter($column_edit);
          }

          if(!_.isEmpty(params.pt_vc_bg_image)) {
